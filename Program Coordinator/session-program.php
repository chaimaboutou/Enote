...
if (isset($_SESSION['idUtilisateur']) && 
    ...
    if ($_SESSION['role'] == 'Program Coordinator') {
       ...
       $programOfCords = getProgramsByIdCoordProgram($id_ProgramCoord, $conn);
       
       $sessions = [];
       
       foreach ($programOfCords as $programOfCord) {
           $program_modules = getModulesByIdProgram($programOfCord['idFiliere'], $conn);
           
           foreach ($program_modules as $program_module) {
               $program_subjects = getSubjectOfModule($program_module['idmodule'], $conn);
               
               foreach ($program_subjects as $program_subject) {
                   $program_sessions = getSessionsOfYearBySubject($id_year, $program_subject['idmatiere'], $conn);
                   $sessions = array_merge($sessions, $program_sessions);
               }
           }
       }
